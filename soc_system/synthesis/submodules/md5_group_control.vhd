...
entity md5_group_control is
	port (
		...
		md5_group_wr           : out std_logic; 
		md5_group_start       : out std_logic_vector(31 downto 0);                    --           .m_start
		md5_group_reset       : out std_logic_vector(31 downto 0);                    --           .m_reset
		md5_group_done        : in  std_logic_vector(31 downto 0) := (others => '0')  --            .m_done
	);
...
begin
		...
		ELSIF(rising_edge(clk))THEN
			...
			ELSIF(avs_s0_read = '1')THEN
				CASE avs_s0_address IS
					...
					WHEN "0010" =>
						avs_s0_readdata <= md5_group_done;
					WHEN OTHERS => 
	...
	md5_group_start <= start;
	md5_group_reset <= reset2;
	md5_group_wr <= wr;

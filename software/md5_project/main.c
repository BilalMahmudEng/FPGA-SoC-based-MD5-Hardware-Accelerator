...
void md5_calc_seq(){
	...
	alt_write_word(md5_group_control +1, 0x00000000); // reset off
	...
	uint32_t md5_input[16] = {
	        ...
	        0x00000000, 0x00000000, 0x00000000, 0x00000000,
	        0x00000000, 0x00000000, 0x00000028,0x00000000}; // 'hello' in little endian
	...
	double TIME1 = start_time.tv_sec * (1000000000) + start_time.tv_nsec; // get start time

	alt_write_word(md5_group_control, 0x1);
	...
	double TIME2 = stop_time.tv_sec * (1000000000) + stop_time.tv_nsec;
   	elapsed_time = TIME2 - TIME1;  // get end time and calculation time
   	uint32_t temp1;
   	uint32_t temp2;
   	uint32_t temp3;
   	uint32_t temp4;
   	uint32_t final_temp;
	for (i =0; i<4;i++){
		alt_write_word(md5_group_data + 2, i); // get a,b,c,d digests
		hashed[i] = alt_read_word(md5_group_data + 3); // get the values
		temp1 = hashed[i] & 0xFF000000;
		temp2 = hashed[i] & 0x00FF0000;
		temp3 = hashed[i] & 0x0000FF00;
		temp4 = hashed[i] & 0x000000FF;
		final_temp = (temp4 << 24)|(temp3 << 8)| (temp2 >> 8) | (temp1>>24); // output is of little endian, converted to big endian
		hashed[i] = final_temp;
	}

	printf(" Digest for '%s' is %08x %08x %08x %08x\nCalculation Time: %0.8f ns", word, hashed[0], hashed[1], hashed[2], hashed[3], elapsed_time);
...
void md5_calc_par(){
	...
	for (core = 0; core < 2; core++){
		for (i =0; i< 16;i++){
			for (j =0; j< 16;j++){
				addr_data = 0x0;
				alt_write_word(md5_group_control + 2, 0x0);
				if (core == 0) alt_write_word(md5_group_data, md5_input1[j]); // writedata
	...
	for (i =0; i< 32;i++){
		for (j = 0 ; j < 4 ; j++){
			...
			hashed3[i][j] = alt_read_word(md5_group_data +3);
			read_data = 0x0;
		}
	}


	printf(" Digest for '%s' is %08x%08x%08x%08x\n", word, hashed3[0][0], hashed3[0][1], hashed3[0][2], hashed3[0][3]);
	printf(" Digest for '%s' is %08x%08x%08x%08x\nnCalculation Time: %0.8f", word2, hashed3[1][0], hashed3[1][1], hashed3[1][2], hashed3[1][3], elapsed_time);

	if (correct_hash[0] == hashed3[0][0] && correct_hash[1] == hashed3[0][1] && correct_hash[2] == hashed3[0][2] && correct_hash[3] == hashed3[0][3]){
		printf("\nThis matches the correct hash value for: %s\n", word);
	}
	else{
		printf("\nThis does not match the correct hash value for: %s\n", word);
	...
	if (correct_hash2[0] == hashed3[1][0] && correct_hash2[1] == hashed3[1][1] && correct_hash2[2] == hashed3[1][2] && correct_hash2[3] == hashed3[1][3]){
		printf("\nThis matches the correct hash value for: %s\n", word2);
	}
	else{
		printf("\nThis does not match the correct hash value for: %s\n", word);
